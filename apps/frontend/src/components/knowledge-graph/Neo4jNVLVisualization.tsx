...
export default function Neo4jNVLVisualization({
  ...
  return (
    <div className="relative w-full h-[90vh]" ref={containerRef}>
      {status === "loading" && (
        ...
        <>
          ...
          {(selection.nodes.size > 0 || selection.relationships.size > 0) && (
            <div className="absolute top-16 left-3 text-[10px] px-2 py-1 rounded bg-accent/20 border border-accent text-foreground font-serif">
              Selected: {selection.nodes.size} node(s),{" "}
              {selection.relationships.size} relationship(s)
            </div>
          )}
            ...
            <div className="absolute bottom-15 right-4 max-w-xs rounded-md bg-card/95 border border-border backdrop-blur-sm shadow-lg p-4">
              ...
              {detailsContent.type === "node" ? (
                <div className="space-y-2">
                  <NodeDetails content={detailsContent} />
                </div>
              ) : (
                <div className="space-y-2">
                  <p className="text-xs font-mono text-muted-foreground">
                    <span className="text-foreground">Type:</span>{" "}
                    {detailsContent.relType}
                  </p>
                  <p className="text-xs font-mono text-muted-foreground">
                    <span className="text-foreground">From:</span>{" "}
                    {detailsContent.data.from}
                  </p>
                  <p className="text-xs font-mono text-muted-foreground">
                    <span className="text-foreground">To:</span>{" "}
                    {detailsContent.data.to}
                  </p>
                  {detailsContent.properties?.when && (
                    <p className="text-xs font-mono text-muted-foreground">
                  ...
                  <div className="pt-2 border-t border-border">
                    {Object.entries(detailsContent.properties || {}).map(
                      ([key, value]) =>
                        key !== "when" && (
                          <p
                            key={key}
